...
public class ServiceManager implements RecordListener<Service> {
    ...
    public void registerInstance(String namespaceId, String serviceName, Instance instance) throws NacosException {
        // 创建实例 ephemeral 是否是临时的实例
        // Nacos的Model是service->cluster->instance这样的，
        // 因此在注册实例时，需要先注册一个Service，具体的操作在createEmptyService函数
     ...
     * 添加实例到nacos
     * 这里再次根据namespaceId以及serviceName获取service对象，并将Instance[]数组注册到service中
     *
     * @param namespaceId namespace 命名空间
     * @param serviceName service name 服务名称
     * @param ephemeral   whether instance is ephemeral 实例是否是临时的
     * @param ips         instances
     * @throws NacosException nacos exception
     */
     ...
     * 这里其实就是更新Instance注册后，需要更新Instance实例的addr信息，
     * 然后将更新后的Instance地址列表信息返回，更新Service下的Instance的列表信息，
    ...
    public void putService(Service service) {
        ...
        if (!serviceMap.containsKey(service.getNamespaceId())) {
            synchronized (putServiceLock) {
                if (!serviceMap.containsKey(service.getNamespaceId())) {
                    // 初始化serviceMap中对应的NamespaceId的map
